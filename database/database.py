...
async def get_fsub_channels() -> List[int]:
    ...
    try:
        ...
        return []
# ============================================
# FSUB JOIN REQUEST MODE CONTROL
# ============================================

async def set_channel_mode(channel_id: int, mode: str):
    """Turn join request mode ON / OFF for FSUB channel."""
    try:
...
async def get_channel_mode(channel_id: int):
    """Get join request mode of FSUB channel."""
    try:
        ch = await fsub_channels_collection.find_one({"channel_id": channel_id})
        return ch.get("mode", "off") if ch else "off"
    except:
        return "off"
